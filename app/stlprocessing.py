...
def slicedweight(pathtofile, density):  
    if platform == 'win32':
        ...
        cmd = r'cd app && chmod -R a+rwx CuraEngine && cd CuraEngine/build && CuraEngine slice -v -j "../resources/definitions/fdmprinter.def.json" -s machine_width=200 -s material_diameter=1.75 -s center_object=true -s adhesion_type=skirt -s infill_line_distance=2.4 -s support_enable=true'
