...
def slicedweight(pathtofile, density):  
    if platform == 'win32':
        cmd = r'cd C:\Program Files\Ultimaker Cura 4.9.0 && CuraEngine slice -v -j "C:\Program Files\Ultimaker Cura 4.9.0\resources\definitions\TL-D3Pro.def.json" -j "C:\Program Files\Ultimaker Cura 4.9.0\resources\definitions\fdmextruder.def.json" -o "C:\Users\abbro\Documents\Visual Studio 2019\Projects\Cubic3D\output.gcode" -s center_object=true -s adhesion_type=skirt -s infill_line_distance=2.4 -s support_enable=false -l "{0}"'.format(pathtofile)
